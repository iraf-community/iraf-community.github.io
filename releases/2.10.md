...
previous: 2.9 · [next: 2.11](2.11) · [index](index)
...
* The new task *hfix* was added to the package. This task allows the user to
...
The KPNOCOUDE package has been customized for the KPNO Coude.  The *doslit*
...
* Two new tasks called *findgain* and *findthresh* were added to this package.
...
* *identify* and *reidentify* now treat multispec format spectra
...
few.  The macro "E" in `<math.h>` was renamed to "BASE_E" to minimize
the chance of an accidental name collision.  The calling sequence for the
*onentry* procedure (ETC) was changed, but since this is a little used
...
Fortran to C translator *f2c* plus the GNU C compiler *gcc* to
...
*f2c* and *gcc*.  The IRAF C code was run through *gcc* with
...
iraf process is started, the process will create two files *pid`.in`
and *pid`.out`, where *pid* is the process id.  Everything read
...
e.g. using a *dparam* file for parameter input, because the full
...
The *xc* program (IRAF compiler-linker) underwent the following changes for
...
before as in previous versions of *xc*.  If the host library is
...
layered package has been changed from *showfloat* to *arch* to
...
and HSI and can be used with host editors such as *vi* to rapidly find
...
with *vi*, start the editor at the IRAF root directory and while in the
...
*ieestat*, and *ieezstat*.  By NaN (not-a-number) we refer
collectively to all IEEE non-arithmetic values, not just IEEE NaN.  The
routines *ieesnan* and *ieegnan* set or get the native floating
...
If NaN mapping is enabled, the *ieestat* routines may be used to
determine the number of input or output NaN conversions occurring since the
last call to *ieezstat*.  Both real and double versions of all routines
are provided.
...
*ieemap* to enable or disable NaN mapping, and *ieezstat* to
...
* The one-dimensional image interpolation library **iminterp** was modified
to add support for sinc interpolation.

* A new library **nlfit** was added for non-linear function fitting.
...
parameter) were renamed *clgpseta* and *clppseta*.  The old
...
* Two new routines *cllpset* and *clepset* were added for listing
...
e.g., the CL, have a special *onentry* procedure which replaces the
...
the host level, but use of an *onentry* procedure allows the in-task
...
```
*gqsort* is identical to the old *qsort* routine except for the
addition of the fourth argument *client_data*.  This is an integer value
which is passed on to the *compare* procedure during sorting to compare
two data values.  The *compare* routine is called as follows.
```
    result = compare (client_data, x1, x2)
```
The new routine eliminates the need to use a common area to pass information
to the compare routine, as was often necessary with *qsort*.

#### FIO interface

The FIO interface (file i/o) underwent minor changes to fix some bugs
affecting pushed back data.  The `F_UNREAD` file status parameter
will now correctly report pushed back data as well as any buffered
input file data.  The `F_CANCEL` file set option will now cancel
...
input text.  The V2.10 release adds two new *printf* output formats,
`%H` and `%M`.  These are used to print numbers in
hours-minutes-seconds or minutes-seconds format and are equivalent to the
older output formats `%h` and `%m` except that the number is
...
format.  In other words, given a number N in units of degrees, `%H`
would print the number in hours-minutes-seconds, i.e., "hh:mm:ss.ss",
whereas `%h` would print the same number as degrees-minutes-seconds,
"dd:mm:ss.ss".  The `%m` formats are similar except that only two of
...
from the file *termcap_file*.  The *ufields* string may be either
...
file.  If *termcap_file* is the null string *ufields* is taken to be
the device entry for the named device.  The *gtycaps* routine may be
used to get the entire device entry as a string, whereas the *gtyget*
and *gtygets* routines are used to get the values of individual
...
add a few new routines.  The *tapecap* facility is new in V2.10.  The
...
Most of these routines are new: the old routines are *mtfile*,
...
magtape file or something else, i.e., a disk file.  *mtneedfileno*
tests whether a file number has been specified in *mtname* (e.g.,
to determine whether or not to query for a file number parameter).
*mtfname* takes *mtname* and a file number and constructs a new
magtape device name in the output string.  *mtparse* parses a magtape
device name into its component parts, and *mtencode* does the reverse.
*mtcap* returns the GTY descriptor of the tapecap entry for the device.
*gtyclose* should be used to free this descriptor when it is no longer
needed.
...
The only new routine here is *mtclean*.  This is called by the
*mtclean* task in the V2.10 SYSTEM package and is used to scan the
...
directory, but in V2.10 the files are stored in `/tmp` so that
...
from node `iraf.noao.edu` in the directory `iraf/v210`, or via
